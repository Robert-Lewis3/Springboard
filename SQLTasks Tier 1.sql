...
FROM (
	...
	LEFT JOIN Facilities AS F ON B.facid = F.facid
	WHERE starttime LIKE '2012-09-14%' 
	ORDER BY Cost DESC) AS Weekly_Bookings
WHERE Cost > 30
...
SELECT  
    M1.firstname, 
    M1.surname,  
    M2.surname AS RecommendedByLN,
    M2.firstname AS RecommendedByFN
FROM Members M1
LEFT OUTER JOIN Members M2 on M1.recommendedby = M2.memid
ORDER BY surname, firstname

/* Q12: Find the facilities with their usage by member, but not guests */

SELECT F.name , COUNT(slots) AS Member_Bookings
FROM Bookings AS B 
LEFT JOIN Facilities AS F ON B.facid = F.facid
WHERE memid != 0
GROUP BY F.name
...
SELECT F.name, MONTH(starttime) AS Month, SUM(slots) AS Member_Bookings
FROM Bookings AS B
	LEFT JOIN Facilities AS F ON B.facid = F.facid
	WHERE memid != 0 
